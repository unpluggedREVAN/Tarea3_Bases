        ...
        BEGIN
			...
			IF EXISTS (SELECT 1 FROM @fechaOperacion WHERE Fecha = @FechaItera)
				BEGIN
					...
					INNER JOIN NuevoTH ON NuevoTH.valorDocId = ntcm.idDocumentoTH
					WHERE NOT EXISTS(
						SELECT 1 FROM NuevosTCM
					...
					INNER JOIN NuevosTCM ON NuevosTCM.idCT = (
						SELECT id
						FROM CuentaTarjetaCredito
						WHERE Codigo = ntca.CodigoCTM AND esMaestra = 1
					)
					INNER JOIN NuevosTH ON NuevosTH.valorDocId = ntca.idDocumentoTH
					WHERE NOT EXISTS(
						SELECT 1 FROM NuevosTCA
						WHERE idCT = CuentaTarjetaCredito.id
							AND idCTM = NuevosCTM.id
							AND idTHPertenece = NuevosTH.id
					)
