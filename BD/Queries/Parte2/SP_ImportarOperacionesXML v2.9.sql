        ...
        BEGIN
			...
			IF EXISTS (SELECT 1 FROM @fechaOperacion WHERE Fecha = @FechaItera)
				BEGIN
					...
					-- Aquí se ejecuta uno de los triggers (EstadodeCuenta)

					-- No necesariamente a la hora de crear cuenta, sino activado por las fechas
					-- Por ahora se activan con los inserts de CuentaTarjetaMaestra y CuentaTarjetaAdicional respectivamente
					...
					-- Aquí se ejecuta el otro trigger (Sub estado de cuenta)
...
--JUAN:
			--Iniciar proceso diario para cada TCM:
				...
				BEGIN
					...
					-- Por ejemplo, se puede actualizar el saldo de la cuenta asociada a la tarjeta física.
